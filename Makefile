...
$(MK_LINTED_SASS): node_modules $(SASS_SOURCES) $(TS_SOURCES)
	...
	npx sass --load-path=src/read-side/html-pages/shared-sass --load-path=src/read-side/html-pages/global-sass --no-source-map src/read-side/html-pages/style.scss:.purgecss/full.css
