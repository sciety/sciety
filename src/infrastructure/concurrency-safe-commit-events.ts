...
const writeEventToDatabase = (pool: Pool) => (event: DomainEvent): T.Task<void> => pipe(
  ...
  }) => [id, type, date, payload, 'List', 'bogus-list-id', '999999'],
  (values) => async () => pool.query(
    'INSERT INTO events (id, type, date, payload, resource_name, resource_id, resource_version) VALUES ($1, $2, $3, $4, $5, $6, $7);',
    values,
  ),
  T.map(() => undefined),
);
...
export const concurrencySafeCommitEvents = (
