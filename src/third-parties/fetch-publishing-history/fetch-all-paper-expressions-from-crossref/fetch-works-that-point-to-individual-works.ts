...
const numberOfRequestedWorks = 1000;
...
const constructUrl = (doi: string) => {
  ...
  return `https://api.crossref.org/works?filter=relation.object:${doi},${workTypes},${relationships}&rows=${numberOfRequestedWorks}`;
};
...
const detectInsufficientNumberOfPagesBeingRequested = (
  ...
  if (response.message['total-results'] > numberOfRequestedWorks) {
