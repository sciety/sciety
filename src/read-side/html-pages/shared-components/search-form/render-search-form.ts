...
const renderCheckbox = (evaluatedOnly: boolean, includeUnevaluatedPreprints: boolean) => {
  if (process.env.EXPERIMENT_ENABLED === 'true') {
    return renderUnevaluatedPreprintsCheckbox(includeUnevaluatedPreprints);
  }
  return renderEvaluatedOnlyCheckbox(evaluatedOnly);
};

export const renderSearchForm = (query: string, evaluatedOnly: boolean, includeUnevaluatedPreprints: boolean): HtmlFragment => toHtmlFragment(`
  <form action="${searchResultsPagePath}" method="get" class="search-form">
    ...
    <div class="search-form__positioning_context">
      ...
      ${renderCheckbox(evaluatedOnly, includeUnevaluatedPreprints)}
