  ...
  TE.tryCatch(
    ...
    (error) => {
      ...
      if (axios.isAxiosError(error)) {
        return `Failed to post evaluation command: ${String(error)}. Response data is: "${JSON.stringify(error.response?.data)}". Evaluation command: ${JSON.stringify(evaluationCommand)}`;
      }
