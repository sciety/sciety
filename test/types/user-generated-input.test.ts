...
describe('user-generated-input', () => {
  it('fails when supplied with script tag', () => {
    const result = userGeneratedInputCodec({ maxInputLength: 100 }).decode('<script>');

    expect(E.isLeft(result)).toBe(true);
  });

  it('passes when supplied with a permitted character é', () => {
    const result = userGeneratedInputCodec({ maxInputLength: 100 }).decode('é');

    expect(E.isRight(result)).toBe(true);
  });

  it('passes when supplied with a permitted special character', () => {
    const result = userGeneratedInputCodec({ maxInputLength: 100 }).decode('Some articles?');

    expect(E.isRight(result)).toBe(true);
  });

  it('fails when supplied with a string that is too long', () => {
    const result = userGeneratedInputCodec({ maxInputLength: 10 }).decode(arbitraryWord(11));
  ...
  describe('when an empty input is not allowed', () => {
    it('fails when supplied with an empty string', () => {
      const result = userGeneratedInputCodec({ maxInputLength: 100 }).decode('');
  ...
  describe('when an empty input is allowed', () => {
    it('does not fail when supplied with an empty string', () => {
      const result = userGeneratedInputCodec({ maxInputLength: 100, allowEmptyInput: true }).decode('');
