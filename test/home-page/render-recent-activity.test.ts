...
describe('render-recent-activity', (): void => {
  ...
  describe('an article is unavailable', () => {
    it('omits the entry', async () => {
      ...
      const fetchArticle: FetchArticle = async () => Maybe.nothing();
      ...
      expect(viewModel[0].articleTitle.isNothing()).toBe(true);
    });
  });
});
