...
describe('get-one-article-id-in-evaluated-state', () => {
  describe('given a biorxiv article that has been evaluated by eLife', () => {
    const articleId = EDOI.fromValidatedString('10.1101/2022.06.22.497259');
    ...
    it('returns one article in evaluated state', () => {
      expect(getOneArticleIdInEvaluatedState(readModel)()).toStrictEqual(O.some(new ArticleId(articleId)));
    });
  });

  describe('given a researchsquare article that has been evaluated by eLife', () => {
    const articleId = EDOI.fromValidatedString('10.21203/rs.3.rs-2407778/v1');
