...
import { constructViewModel } from '../../../../../src/read-side/html-pages/groups-page-alt/construct-view-model/construct-view-model';
import { GroupCardViewModel } from '../../../../../src/read-side/html-pages/shared-components/group-card/view-model';
  ...
  describe('when there is more than one group', () => {
    ...
    beforeEach(async () => {
      ...
      groupNames = await pipe(
        ...
        T.map((viewModel) => pipe(
          viewModel.groupCards,
          RA.map((groupCard) => groupCard.name),
        )),
      )();
    });
