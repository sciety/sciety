...
void (async (): Promise<void> => {
  ...
  const { data } = await axios.get<CrossrefResponse>('https://api.crossref.org/prefixes/10.7287/works?filter=type:peer-review,relation.object:10.7717/peerj.434', { headers: { 'User-Agent': 'TheHive (http://hive.review; mailto:team@hive.review)' } });
  process.stdout.write('Date,Article DOI,Review ID\n');
  const [year, month, day] = data.message.items[0]['published-print']['date-parts'][0];
  const date = new Date(year, month - 1, day);
  const articleDoi = biorxivResponseFirstCollectionItem.biorxiv_doi;
  const reviewDoi = data.message.items[0].DOI;
